...
const postPosts = async (req,res,next) =>{
  try {
    let success = await model.createPost(req.body);
    if(success === true){
      res.send(`"Blog with the id of : " ${req.body.Id} 'was created successfully' `);
    }
...
const updatePost = async (req,res,next) =>{
  try{
   let success = await model.updatePost(req.body);
   if(success){
...
const deletePost = async (req,res,next) =>{
  try{
    let success = await model.deletePost(req.params.Id);
    if(success){
